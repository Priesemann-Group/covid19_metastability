function [Nobs_total,vacc_frac,Imm_frac,ne,ts,Ncrit] = dailyCases_vaccine_SEIR(sol,M,xi,tc,tau,nu,Rt,Gamma,lambda_s,lambda_r,eta,epsilon,nmax,Phi0,Phild,T1,D_LD,D,Rtld,Rtald,phi,lambda_s2,lambda_r2,R0,V)
...
Nobs_total  = EstimDelay_num((1/tc) * ET + Ntest + fT*ne);
Ncrit       = Nobs_total(iNcrit);
vacc_frac   = acumula(t,dvacdt(t,V))/(M*V(3));
Imm_frac    = 1-S/M;
