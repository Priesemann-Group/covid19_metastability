...
paso = 2;
flag = 0;
Rt_LockDown = 0:0.01:0.45;%0.99*Rtcmax2/R0;
T1 = 0:1:10*7;
...
%% Solo real 
[X,Y] = meshgrid(Rt_LockDown,T1/7);
ax = backplot('Fig~0: Opt. LD duration');
% plot(100*Rt_LockDown/R0,DOPT/7,lt,'Color',bl1,'LineWidth',3*fact_curva)
hold on
surf(X,Y,DOPT'/7)
colorbar
...
hold on
xlabel('$k_{\rm LD}$','interpreter','latex','FontSize',10*fact_label) 
ylabel('Start of lockdown $t_{s}$','interpreter','latex','FontSize',10*fact_label)
hold on
ylim([0 T1(end)/7])
set(gca,'YTick',[0 2 4])
xlim([0 0.6])
