...
xim = 1-xi;
Rtdef = fzero(@(Rt) maxvpdde_lin(xi,tc,tau,nu,Rt,Gamma,lambda_s,lambda_r,eta,epsilon,R0),2);
Rtdef0 = fzero(@(Rt) maxvpdde_lin(xi,tc,tau,nu,Rt,Gamma,lambda_s2,lambda_r2,0,epsilon,R0),2);
...
for i = 1:N_sampling
    try
    ktcrit_TTI(i) = (1/R0)*fzero(@(Rt) maxvpdde_lin(Xi(i),tc,Tau(i),Nu(i),Rt,Gamma,Lambda_s(i),lambda_r,Eta(i),Epsilon(i),R0),2);
    catch err
        ktcrit_TTI(i) = NaN;
    end
    try
    ktcrit_Hosp(i) = (1/R0)*fzero(@(Rt) maxvpdde_lin(Xi(i),tc,Tau(i),Nu(i),Rt,Gamma,lambda_s2,lambda_r,0,Epsilon(i),R0),2);
    catch err
        ktcrit_Hosp(i) = NaN;
    end
end
