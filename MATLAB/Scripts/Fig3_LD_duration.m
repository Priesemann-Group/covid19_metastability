...
% id = [10 50 106 200 300 400 407];
N_delays = 7;
Nobs_rep = NaN(length(ti:tf),N_delays);
idx = Tdis>=-1 & Tdis<=12*7; nt = sum(idx);
Cumulative_cases = NaN(nt,N_delays);
Rthat = NaN(length(ts),N_delays);
for k = 1:N_delays
    id(k)= floor(512*(k/(9 + 1)));%448
    if k == 1
        tk = t1; tk2 = t1+D;
...
ax = backplot('Fig 1: LD duration');
plot(0:6,Cumulative_cases(end,:),'*','Color',red2,'LineWidth',3*fact_curva)
hold on
plot(1:6,Cumulative_cases(end,2:end),'-','Color',red2,'LineWidth',1*fact_curva)
hold on
set(gca,'FontSize',15*fact_axis)
hold on
xlabel('Weeks','interpreter','latex','FontSize',15*fact_label)
...
ax = backplot('Fig 1: LD duration');
kt_profiles = NaN(length(ts),N_delays);
for k = 1:N_delays
