...
%% Find the point where we cross the threshold
...
id = find(Nobs_rep>=Thrs1,1,'first');
T1 = ts(id)+delay;
flag = 1;
...
while flag
    sol = dde23(@(t,x,Z) dde_vaccine_SEIR(t,x,Z,M,xi,tc,tau,nu,Rt,Gamma,lambda_s,lambda_r,eta,epsilon,nmax,Phi0,Phild,T1,D_LD,D,Rtld,Rtald,phi,lambda_s2,lambda_r2,R0,V), lags, @(t) history(t,x0), tspan,options);
    [Nobs_rep,~,~,~,ts,~] = dailyCases_vaccine_SEIR(sol,M,xi,tc,tau,nu,Rt,Gamma,lambda_s,lambda_r,eta,epsilon,nmax,Phi0,Phild,T1,D_LD,D,Rtld,Rtald,phi,lambda_s2,lambda_r2,R0,V);
    Tdis = ts;
...
[Nobs_rep,vacc_frac,Imm_frac,ne,ts,Neqcrit] = dailyCases_vaccine_SEIR(sol,M,xi,tc,tau,nu,Rt,Gamma,lambda_s,lambda_r,eta,epsilon,nmax,Phi0,Phild,T1,D_LD,D,Rtld,Rtald,phi,lambda_s2,lambda_r2,R0,V);
Tdis = ts;
...
ax = backplot('Daily new cases');
plot(Tdis/7,Imm_frac,lt,'Color',or2,'LineWidth',3*fact_curva)
hold on
plot(Tdis/7,vacc_frac,lt,'Color','g','LineWidth',1.5*fact_curva)
hold on
set(gca,'FontSize',15*fact_axis)
hold on
xlabel('weeks since lockdown','interpreter','latex','FontSize',15*fact_label) 
ylabel('fraction','interpreter','latex','FontSize',15*fact_label)
hold on
xlim([tmin tmax]/7)
ylim([0 1])
ax.TickLabelInterpreter='latex';
